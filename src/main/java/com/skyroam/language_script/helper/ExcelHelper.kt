package com.skyroam.language_script.helper

import com.skyroam.language_script.bean.CsvBean
...
//                Log.d(TAG, "工作表[" + sheetName + "]的第" + rowNum + "行第" + colNum + "列的值为：" + cellData)
            ...
            e.printStackTrace()
            Log.e(TAG, "工作表[" + sheetName + "]的第" + rowNum + "行第" + colNum + "列的值时异常：" + e)
        }
        return ""
    }
    ...
    private fun getCell(row: XSSFRow, colNum: Int): XSSFCell {
        return row.getCell(colNum, Row.MissingCellPolicy.RETURN_BLANK_AS_NULL) ?: row.createCell(colNum)
    }
